    ...
    if (cfg[weaponIndex].enabled && (cmd->buttons & UserCmd::IN_ATTACK || cfg[weaponIndex].aimlock)) {
        ...
        for (int i = 1; i <= interfaces->engine->getMaxClients(); i++) {
            ...
            for (size_t j = 0; j < hitbox.size(); j++)
            {
                ...
                for (auto& bonePosition : hitscan::multiPoint(entity, player.matrix.data(), hitbox, localPlayerEyePosition, j, 0))
                {
                    ...
                    if (!entity->isVisible(bonePosition) && (cfg[weaponIndex].visibleOnly || !hitscan::canScan(entity, bonePosition, activeWeapon->getWeaponData(), cfg[weaponIndex].killshot ? entity->health() : cfg[weaponIndex].minDamage, cfg[weaponIndex].friendlyFire)))
                        continue;
